      ...
      <div className="w-72 shrink-0 bg-white border-r border-slate-100 flex flex-col overflow-hidden">
        <div className="p-6 border-b border-slate-100">
          ...
          <select
            ...
            className="w-full text-xs font-bold p-3 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:border-[#7CB342] transition-colors"
          >
          ...
          {entities.map(e => (
            ...
            >
              <div className="flex items-center gap-3">
                <Database size={13} className={selectedEntityId === e.id ? 'text-[#7CB342]' : 'text-slate-300'} />
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-slate-50/50 min-w-0">
        ...
        <header className="px-8 py-4 bg-white border-b border-slate-100 flex items-center justify-between shrink-0 shadow-sm z-10 gap-4">
          <div className="flex items-center gap-3 flex-1 max-w-xl">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-100 rounded-2xl text-xs font-bold outline-none focus:border-[#7CB342] transition-all"
              />
              {searchQuery && (
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              ...
              className={`p-2.5 rounded-xl transition-all relative ${showColumnFilters ? 'bg-[#7CB342] text-white' : 'bg-slate-50 text-slate-400 hover:bg-slate-100 border border-slate-100'}`}
            >
        ...
        {cwpColumn && cwpValues.length > 0 && (
          <div className="px-8 py-2.5 bg-white border-b border-slate-100 flex items-center gap-2.5 overflow-x-auto shrink-0">
            <div className="flex items-center gap-2 shrink-0">
              <Tag size={11} className="text-[#7CB342]" />
        ...
        {selectedEntityId && (
          <div className="px-8 py-2.5 flex items-center justify-between border-b border-slate-100 bg-white/50 text-[10px] font-black uppercase tracking-widest text-slate-400 shrink-0">
            <div className="flex items-center gap-5">
              <span className="flex items-center gap-1.5"><Layers size={11} className="text-[#7CB342]" /> {data.length} Total</span>
                    ...
                    <div className="flex items-center gap-4 px-4">
                      <button
                        ...
                        className={`p-1.5 rounded-xl transition-all ${
                          groupRows.every(r => selectedRows.has(r.id)) ? 'bg-[#7CB342] text-white' : 'bg-white text-slate-300 border border-slate-200'
                        }`}
                      >
                        {groupRows.every(r => selectedRows.has(r.id)) ? <CheckSquare size={14} /> : <Square size={14} />}
                      </button>
                      <div className="h-3.5 w-1 bg-[#7CB342] rounded-full" />
                      <h3 className="text-xs font-black text-slate-700 uppercase tracking-widest">
                        {groupBy}: <span className="text-[#7CB342] italic">{groupKey}</span>
                      </h3>
                          ...
                          <tr>
                            <th className="px-5 py-4 w-10 border-b border-slate-100 sticky left-0 bg-slate-50/90">
                              <button onClick={selectAll} className="p-1 hover:bg-slate-200 rounded-lg transition-colors">
                                {selectedRows.size === filteredData.length && filteredData.length > 0
                                  ? <CheckSquare size={16} className="text-[#7CB342]" />
                            ...
                            {visibleColumns.map(col => (
                              <th key={col} className={`px-5 py-4 text-[10px] font-black uppercase tracking-widest border-b border-slate-100 whitespace-nowrap ${
                                col === cwpColumn ? 'text-[#7CB342]' : 'text-slate-400'
                              }`}>
                                ...
                                <td key={col} className="px-3 py-2">
                                  <input
                                    ...
                                    className={`w-full px-2.5 py-1.5 bg-slate-50 border rounded-lg text-[9px] font-medium outline-none transition-all ${
                                      columnFilters[col] ? 'border-[#7CB342] bg-green-50/30' : 'border-slate-200 focus:border-[#7CB342]'
                                    }`}
                                  />
                                </td>
                              ))}
                              ...
                              >
                                <td className="px-5 py-3 sticky left-0 bg-inherit">
                                  <button
                                    ...
                                    className={`p-1 rounded-lg transition-colors ${isSelected ? 'text-[#7CB342]' : 'text-slate-200 group-hover:text-slate-300'}`}
                                  >
                                  ...
                                  <td key={col} className="px-5 py-3">
                                    <input
                                      ...
                                      className={`w-full bg-transparent border-none outline-none text-xs transition-all font-medium min-w-[80px] ${
                                        isEdited ? 'text-amber-700 font-bold' : col === cwpColumn ? 'text-[#7CB342] font-bold' : 'text-slate-600'
                                      } hover:bg-slate-100 p-1.5 rounded-lg focus:bg-white focus:shadow-inner focus:ring-1 focus:ring-[#7CB342]/20`}
                                    />
                                  </td>
                                ))}
                              </tr>
          ...
          <div className="px-3 py-2 border-b border-slate-50">
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 py-1.5 text-[9px] font-black text-[#7CB342] uppercase tracking-wider hover:bg-green-50 rounded-lg transition-all"
              >
                Mostrar Todas
              </button>
              <button
              ...
              return (
                ...
                >
                  {isHidden
                    ...
                    : <Eye size={11} className={col === cwpColumn ? 'text-[#7CB342] shrink-0' : 'text-slate-400 shrink-0'} />
                  }
                  <span className={`truncate ${col === cwpColumn ? 'text-[#7CB342] font-black' : ''}`}>{col}</span>
                </button>
              );
            })}
          </div>
