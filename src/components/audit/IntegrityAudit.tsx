            ...
            <div key={rel.id} className="bg-white rounded-[3rem] border border-slate-100 shadow-sm overflow-hidden group hover:shadow-2xl transition-all duration-500">
              ...
              <div className="p-8 border-b border-slate-50 bg-slate-50/20 flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="flex items-center gap-6">
                   <div className="w-14 h-14 bg-white border-2 border-[#7CB342]/10 rounded-2xl flex items-center justify-center text-[#7CB342] shadow-sm transform group-hover:scale-110 transition-transform">
                     <Network size={28} />
                   </div>
                   <div>
                     <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Conexión establecida por: <span className="text-[#7CB342]">{rel.parent_attr?.name} = {rel.child_attr?.name}</span></p>
                ...
                <div className="flex items-center gap-4">
                  {auditProgress[`${rel.id}-parent`] || auditProgress[`${rel.id}-child`] ? (
                    <div className="flex items-center gap-2 text-[10px] font-black text-[#7CB342] animate-pulse">
                      <Database size={12} />
                      {auditProgress[`${rel.id}-parent`] || auditProgress[`${rel.id}-child`]}
                    </div>
                  ) : null}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 divide-x divide-slate-100">
                ...
                <div className="p-10 space-y-8 bg-white">
                  <div className="flex justify-between items-center">
                    <h5 className="text-[12px] font-black text-slate-900 uppercase tracking-tighter flex items-center gap-2">
                       <span className="text-[#7CB342] truncate max-w-[120px]">{res.parentEntityName}</span>
                       <ArrowRight size={14} className="text-slate-300" />
                       <span className="text-slate-400 truncate max-w-[120px]">{res.childEntityName}</span>
                    </h5>
                    <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${pIntegrity > 95 ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-red-50 text-red-600 border border-red-100'}`}>
                  ...
                  <div className="grid grid-cols-2 gap-8">
                    <div className="space-y-1">
                      <p className="text-4xl font-black text-emerald-600 italic tracking-tighter">{pMatches}</p>
                ...
                <div className="p-10 space-y-8 bg-slate-50/30">
                  <div className="flex justify-between items-center">
                    <h5 className="text-[12px] font-black text-slate-900 uppercase tracking-tighter flex items-center gap-2">
                       <span className="text-[#7CB342] truncate max-w-[120px]">{res.childEntityName}</span>
                       <ArrowRight size={14} className="text-slate-300" />
                       <span className="text-slate-400 truncate max-w-[120px]">{res.parentEntityName}</span>
                    </h5>
                    <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${cIntegrity > 95 ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-red-50 text-red-600 border border-red-100'}`}>
                  ...
                  <div className="grid grid-cols-2 gap-8">
                    <div className="space-y-1">
                      <p className="text-4xl font-black text-emerald-600 italic tracking-tighter">{cMatches}</p>
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Coincidencias (Matches)</p>
                    </div>
                    <div className="space-y-1">
                      <div className="flex items-center gap-3">
                        ...
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-xl text-[9px] font-black uppercase whitespace-nowrap transition-all border ${
                            !hiddenColumns.has(col) 
                              ? 'bg-[#7CB342]/10 text-[#7CB342] border-[#7CB342]/20' 
             ...
             <div className="flex-1 overflow-auto p-8 bg-slate-50/30">
               {selectedOrphanView.orphans.length > 0 ? (
                     ...
                     <thead className="bg-slate-50 sticky top-0 z-10">
                       <tr>
                         <th className="px-6 py-4 w-10">
                           ...
                           }} className="text-slate-300 hover:text-[#7CB342]">
                                ...
                                {editingCell?.recordId === row.__record_id && editingCell?.field === col ? (
                                  <div className="absolute inset-0 z-20 p-2 bg-white">
                                    <input 
                                      ...
                                      className="w-full h-full px-2 border-2 border-[#7CB342] rounded-lg text-xs font-bold focus:outline-none"
                                    />
                                  </div>
                                ) : (
                 ...
                 <div className="h-full flex flex-col items-center justify-center text-center space-y-4">
                    ...
                    <p className="text-emerald-700 font-black italic text-xl">Sin huérfanos detectados</p>
