...
export default function RelationalExplorer({ entities, relationships, onRefresh }: RelationalExplorerProps) {
  ...
  const getColumnColor = (col: string) => {
    if (!col.includes('::')) return col === cwpColumn ? 'bg-green-50 text-[#7CB342]' : 'bg-slate-100 text-slate-600';
      ...
      <div className="w-72 shrink-0 bg-white border-r border-slate-100 flex flex-col overflow-hidden">
        <div className="p-5 border-b border-slate-100 space-y-3">
          ...
          <select
            ...
            className="w-full text-xs font-bold p-3 bg-slate-50 border border-slate-100 rounded-xl outline-none focus:border-[#7CB342] transition-colors"
          >
          ...
          {baseEntityId && (
            <div className="relative">
              ...
              <input
                ...
                className="w-full text-[10px] font-bold pl-8 pr-4 py-2 bg-slate-50 border border-slate-100 rounded-lg outline-none focus:border-[#7CB342]"
              />
            </div>
          )}
        </div>
            ...
            <div className="p-4 border-b border-slate-50">
              ...
              <div className="space-y-0.5">
                {baseColumns.filter(c => !c.startsWith('_batch')).map(col => (
                  ...
                  >
                    <span className={`truncate flex items-center gap-1.5 ${['CWP','PACKAGE','PAQUETE'].includes(col.toUpperCase().trim()) ? 'text-[#7CB342]' : ''}`}>
            ...
            <div className="flex items-center gap-3 flex-1">
              ...
              <div className="relative flex-1 max-w-sm">
                ...
                <input
                  ...
                  className="w-full pl-8 pr-8 py-2 bg-slate-50 border border-slate-100 rounded-xl text-[10px] font-bold outline-none focus:border-[#7CB342]"
                />
                {tableFilter && (
              ...
              {cwpColumn && cwpValues.length > 0 && (
                <div className="flex items-center gap-2 overflow-x-auto max-w-md">
                  <Tag size={11} className="text-[#7CB342] shrink-0" />
                  <button
                  ...
                  {cwpValues.slice(0, 8).map(val => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-full text-[9px] font-black shrink-0 transition-all ${cwpFilter === val ? 'bg-[#7CB342] text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}
                    >
                      {val}
                    </button>
                  ))}
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              {filteredJoinedRows.length > 0 && (
                <button
                  ...
                  className="px-4 py-2 bg-emerald-50 text-emerald-600 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 border border-emerald-100 hover:bg-emerald-100 transition-all"
                >
                      ...
                      <tr key={idx} className="hover:bg-slate-50/80 transition-colors">
                        {allDisplayColumns.map(col => (
                          <td key={col} className="px-4 py-2.5 max-w-[200px]">
                            <span className={`block truncate ${!row[col] || row[col] === '—' ? 'text-slate-300 italic text-[9px]' : col === cwpColumn ? 'text-[#7CB342] font-bold text-[11px]' : 'text-slate-700'}`}>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  autoFocus
                  className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold outline-none focus:border-[#7CB342] transition-all"
                />
              </div>
              ...
              <div className="p-4 bg-slate-50 rounded-2xl space-y-1.5">
                ...
                {cwpColumn && <p className="text-[11px] font-bold text-[#7CB342]">Filtro CWP detectado: <span className="font-black">{cwpColumn}</span></p>}
              </div>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className={`flex-1 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${
                  saveSuccess ? 'bg-[#7CB342] text-white' : 'bg-slate-900 text-white hover:bg-slate-800'
                } disabled:opacity-50`}
              >
